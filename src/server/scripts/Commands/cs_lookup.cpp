...
public:
    ...
    static bool HandleLookupAreaCommand(ChatHandler* handler, char const* args)
    {
        ...
        for (uint32 areaflag = 0; areaflag < sAreaStore.GetNumRows(); ++areaflag)
        {
            ...
            if (areaEntry)
            {
                ...
                if (locale < TOTAL_LOCALES)
                {
                    ...
                    {
                        handler->PSendSysMessage(LANGUAGE_COMMAND_LOOKUP_MAX_RESULTS, maxResults);
                        return true;
                    }
        ...
        if (!found)
            handler->SendSysMessage(LANGUAGE_COMMAND_NOAREAFOUND);
    ...
    static bool HandleLookupCreatureCommand(ChatHandler* handler, char const* args)
    {
            ...
            if (CreatureLocale const* creatureLocale = sObjectMgr->GetCreatureLocale(id))
            {
                if (creatureLocale->Name.size() > localeIndex && !creatureLocale->Name[localeIndex].empty())
                {
                    ...
                    if (Utf8FitTo(name, wNamePart))
                    {
                        ...
                        if (handler->GetSession())
                            handler->PSendSysMessage(LANGUAGE_CREATURE_ENTRY_LIST_CHAT, id, id, name.c_str());
                        else
                            handler->PSendSysMessage(LANGUAGE_CREATURE_ENTRY_LIST_CONSOLE, id, name.c_str());
        ...
        if (!found)
            handler->SendSysMessage(LANGUAGE_COMMAND_NOCREATUREFOUND);
    ...
    static bool HandleLookupEventCommand(ChatHandler* handler, char const* args)
    {
        ...
        for (uint32 id = 0; id < events.size(); ++id)
        {
            ...
            if (Utf8FitTo(descr, wNamePart))
            {
                ...
                char const* active = activeEvents.find(id) != activeEvents.end() ? handler->GetSkyFireString(LANGUAGE_ACTIVE) : "";

                if (handler->GetSession())
                    handler->PSendSysMessage(LANGUAGE_EVENT_ENTRY_LIST_CHAT, id, id, eventData.description.c_str(), active);
                else
                    handler->PSendSysMessage(LANGUAGE_EVENT_ENTRY_LIST_CONSOLE, id, eventData.description.c_str(), active);
        ...
        if (!found)
            handler->SendSysMessage(LANGUAGE_NOEVENTFOUND);
    ...
    static bool HandleLookupFactionCommand(ChatHandler* handler, char const* args)
    {
                ...
                if (locale < TOTAL_LOCALES)
                {
                    ...
                    if (factionState) // and then target != NULL also
                    {
                        ...
                        if (factionState->Flags & FACTION_FLAG_VISIBLE)
                            ss << handler->GetSkyFireString(LANGUAGE_FACTION_VISIBLE);
                        if (factionState->Flags & FACTION_FLAG_AT_WAR)
                            ss << handler->GetSkyFireString(LANGUAGE_FACTION_ATWAR);
                        if (factionState->Flags & FACTION_FLAG_PEACE_FORCED)
                            ss << handler->GetSkyFireString(LANGUAGE_FACTION_PEACE_FORCED);
                        if (factionState->Flags & FACTION_FLAG_HIDDEN)
                            ss << handler->GetSkyFireString(LANGUAGE_FACTION_HIDDEN);
                        if (factionState->Flags & FACTION_FLAG_INVISIBLE_FORCED)
                            ss << handler->GetSkyFireString(LANGUAGE_FACTION_INVISIBLE_FORCED);
                        if (factionState->Flags & FACTION_FLAG_INACTIVE)
                            ss << handler->GetSkyFireString(LANGUAGE_FACTION_INACTIVE);
                    }
                    else
                        ss << handler->GetSkyFireString(LANGUAGE_FACTION_NOREPUTATION);
        ...
        if (!found)
            handler->SendSysMessage(LANGUAGE_COMMAND_FACTION_NOTFOUND);
        return true;
    }
    ...
    static bool HandleLookupItemCommand(ChatHandler* handler, char const* args)
    {
                ...
                if (ItemLocale const* il = sObjectMgr->GetItemLocale(itr->second.ItemId))
                {
                    if (il->Name.size() > ulocaleIndex && !il->Name[ulocaleIndex].empty())
                    {
                        ...
                        if (Utf8FitTo(name, wNamePart))
                        {
                            ...
                            if (handler->GetSession())
                                handler->PSendSysMessage(LANGUAGE_ITEM_LIST_CHAT, itr->second.ItemId, itr->second.ItemId, name.c_str());
                            else
                                handler->PSendSysMessage(LANGUAGE_ITEM_LIST_CONSOLE, itr->second.ItemId, name.c_str());
        ...
        if (!found)
            handler->SendSysMessage(LANGUAGE_COMMAND_NOITEMFOUND);
    ...
    static bool HandleLookupItemSetCommand(ChatHandler* handler, char const* args)
    {
        ...
        for (uint32 id = 0; id < sItemSetStore.GetNumRows(); id++)
        {
            ...
            if (set)
            {
                ...
                if (locale < TOTAL_LOCALES)
                {
                    ...
                    if (handler->GetSession())
                        handler->PSendSysMessage(LANGUAGE_ITEMSET_LIST_CHAT, id, id, name.c_str(), localeNames[locale]);
                    else
                        handler->PSendSysMessage(LANGUAGE_ITEMSET_LIST_CONSOLE, id, name.c_str(), localeNames[locale]);
        ...
        if (!found)
            handler->SendSysMessage(LANGUAGE_COMMAND_NOITEMSETFOUND);
    ...
    static bool HandleLookupObjectCommand(ChatHandler* handler, char const* args)
    {
            ...
            if (GameObjectLocale const* objectLocalte = sObjectMgr->GetGameObjectLocale(itr->second.entry))
            {
                if (objectLocalte->Name.size() > localeIndex && !objectLocalte->Name[localeIndex].empty())
                {
                    ...
                    if (Utf8FitTo(name, wNamePart))
                    {
                        ...
                        if (handler->GetSession())
                            handler->PSendSysMessage(LANGUAGE_GO_ENTRY_LIST_CHAT, itr->second.entry, itr->second.entry, name.c_str());
                        else
                            handler->PSendSysMessage(LANGUAGE_GO_ENTRY_LIST_CONSOLE, itr->second.entry, name.c_str());
        ...
        if (!found)
            handler->SendSysMessage(LANGUAGE_COMMAND_NOGAMEOBJECTFOUND);
    ...
    static bool HandleLookupQuestCommand(ChatHandler* handler, char const* args)
    {
                ...
                if (QuestLocale const* questLocale = sObjectMgr->GetQuestLocale(qInfo->GetQuestId()))
                {
                    if (questLocale->Title.size() > ulocaleIndex && !questLocale->Title[ulocaleIndex].empty())
                    {
                        ...
                        if (Utf8FitTo(title, wNamePart))
                        {
                            ...
                            if (target)
                            {
                                ...
                                switch (status)
                                {
                                    case QUEST_STATUS_COMPLETE:
                                        statusStr = handler->GetSkyFireString(LANGUAGE_COMMAND_QUEST_COMPLETE);
                                        break;
                                    case QUEST_STATUS_INCOMPLETE:
                                        statusStr = handler->GetSkyFireString(LANGUAGE_COMMAND_QUEST_ACTIVE);
                                        break;
                                    case QUEST_STATUS_REWARDED:
                                        statusStr = handler->GetSkyFireString(LANGUAGE_COMMAND_QUEST_REWARDED);
                                        break;
                                    default:
                                        break;
                                }
                            ...
                            if (handler->GetSession())
                                handler->PSendSysMessage(LANGUAGE_QUEST_LIST_CHAT, qInfo->GetQuestId(), qInfo->GetQuestId(), qInfo->GetQuestLevel(), title.c_str(), statusStr);
                            else
                                handler->PSendSysMessage(LANGUAGE_QUEST_LIST_CONSOLE, qInfo->GetQuestId(), title.c_str(), statusStr);
        ...
        if (!found)
            handler->SendSysMessage(LANGUAGE_COMMAND_NOQUESTFOUND);
    ...
    static bool HandleLookupSkillCommand(ChatHandler* handler, char const* args)
    {
        ...
        for (uint32 id = 0; id < sSkillLineStore.GetNumRows(); id++)
        {
            ...
            if (skillInfo)
            {
                ...
                if (locale < TOTAL_LOCALES)
                {
                    ...
                    {
                        knownStr = handler->GetSkyFireString(LANGUAGE_KNOWN);
                        uint32 curValue = target->GetPureSkillValue(id);
                        uint32 maxValue  = target->GetPureMaxSkillValue(id);
                        uint32 permValue = target->GetSkillPermBonusValue(id);
                        uint32 tempValue = target->GetSkillTempBonusValue(id);

                        char const* valFormat = handler->GetSkyFireString(LANGUAGE_SKILL_VALUES);
                        snprintf(valStr, 50, valFormat, curValue, maxValue, permValue, tempValue);
                    }

                    // send skill in "id - [namedlink locale]" format
                    if (handler->GetSession())
                        handler->PSendSysMessage(LANGUAGE_SKILL_LIST_CHAT, id, id, name.c_str(), localeNames[locale], knownStr, valStr);
                    else
                        handler->PSendSysMessage(LANGUAGE_SKILL_LIST_CONSOLE, id, name.c_str(), localeNames[locale], knownStr, valStr);
        ...
        if (!found)
            handler->SendSysMessage(LANGUAGE_COMMAND_NOSKILLFOUND);
    ...
    static bool HandleLookupSpellCommand(ChatHandler* handler, char const* args)
    {
        ...
        for (uint32 id = 0; id < sSpellMgr->GetSpellInfoStoreSize(); id++)
        {
            ...
            if (spellInfo)
            {
                ...
                if (locale < TOTAL_LOCALES)
                {
                    ...
                    if (rank)
                        ss << handler->GetSkyFireString(LANGUAGE_SPELL_RANK) << rank;
                    ...
                    if (talent)
                        ss << handler->GetSkyFireString(LANGUAGE_TALENT);
                    if (passive)
                        ss << handler->GetSkyFireString(LANGUAGE_PASSIVE);
                    if (learn)
                        ss << handler->GetSkyFireString(LANGUAGE_LEARN);
                    if (known)
                        ss << handler->GetSkyFireString(LANGUAGE_KNOWN);
                    if (active)
                        ss << handler->GetSkyFireString(LANGUAGE_ACTIVE);
        ...
        if (!found)
            handler->SendSysMessage(LANGUAGE_COMMAND_NOSPELLFOUND);
    ...
    static bool HandleLookupTaxiNodeCommand(ChatHandler* handler, const char * args)
    {
        ...
        for (uint32 id = 0; id < sTaxiNodesStore.GetNumRows(); id++)
        {
            ...
            if (nodeEntry)
            {
                ...
                if (locale < TOTAL_LOCALES)
                {
                    ...
                    if (handler->GetSession())
                        handler->PSendSysMessage(LANGUAGE_TAXINODE_ENTRY_LIST_CHAT, id, id, name.c_str(), localeNames[locale],
                            nodeEntry->map_id, nodeEntry->x, nodeEntry->y, nodeEntry->z);
                    else
                        handler->PSendSysMessage(LANGUAGE_TAXINODE_ENTRY_LIST_CONSOLE, id, name.c_str(), localeNames[locale],
                            nodeEntry->map_id, nodeEntry->x, nodeEntry->y, nodeEntry->z);
        ...
        if (!found)
            handler->SendSysMessage(LANGUAGE_COMMAND_NOTAXINODEFOUND);
    ...
    static bool HandleLookupTeleCommand(ChatHandler* handler, char const* args)
    {
        if (!*args)
        {
            handler->SendSysMessage(LANGUAGE_COMMAND_TELE_PARAMETER);
        ...
        if (reply.str().empty())
            handler->SendSysMessage(LANGUAGE_COMMAND_TELE_NOLOCATION);
        else
            handler->PSendSysMessage(LANGUAGE_COMMAND_TELE_LOCATION, reply.str().c_str());
    ...
    static bool HandleLookupTitleCommand(ChatHandler* handler, char const* args)
    {
        ...
        for (uint32 id = 0; id < sCharTitlesStore.GetNumRows(); id++)
        {
            ...
            if (titleInfo)
            {
                ...
                if (locale < TOTAL_LOCALES)
                {
                    ...
                    char const* knownStr = target && target->HasTitle(titleInfo) ? handler->GetSkyFireString(LANGUAGE_KNOWN) : "";

                    char const* activeStr = target && target->GetUInt32Value(PLAYER_CHOSEN_TITLE) == titleInfo->bit_index
                        ? handler->GetSkyFireString(LANGUAGE_ACTIVE)
                        : "";
                    ...
                    if (handler->GetSession())
                        handler->PSendSysMessage(LANGUAGE_TITLE_LIST_CHAT, id, titleInfo->bit_index, id, titleNameStr, localeNames[locale], knownStr, activeStr);
                    else
                        handler->PSendSysMessage(LANGUAGE_TITLE_LIST_CONSOLE, id, titleInfo->bit_index, titleNameStr, localeNames[locale], knownStr, activeStr);
        ...
        if (counter == 0)  // if counter == 0 then we found nth
            handler->SendSysMessage(LANGUAGE_COMMAND_NOTITLEFOUND);
    ...
    static bool HandleLookupMapCommand(ChatHandler* handler, char const* args)
    {
            ...
            if (MapEntry const* mapInfo = sMapStore.LookupEntry(id))
            {
                ...
                if (Utf8FitTo(name, wNamePart) && locale < TOTAL_LOCALES)
                {
                    ...
                    if (mapInfo->IsContinent())
                        ss << handler->GetSkyFireString(LANGUAGE_CONTINENT);

                    switch (mapInfo->map_type)
                    {
                        case MAP_INSTANCE:
                            ss << handler->GetSkyFireString(LANGUAGE_INSTANCE);
                            break;
                        case MAP_RAID:
                            ss << handler->GetSkyFireString(LANGUAGE_RAID);
                            break;
                        case MAP_BATTLEGROUND:
                            ss << handler->GetSkyFireString(LANGUAGE_BATTLEGROUND);
                            break;
                        case MAP_ARENA:
                            ss << handler->GetSkyFireString(LANGUAGE_ARENA);
                            break;
                    }
        ...
        if (!counter)
            handler->SendSysMessage(LANGUAGE_COMMAND_NOMAPFOUND);
    ...
    static bool HandleLookupPlayerIpCommand(ChatHandler* handler, char const* args)
    {
        ...
        PreparedStatement* stmt = LoginDatabase.GetPreparedStatement(LOGIN_SELECT_ACCOUNT_BY_IP);
        stmt->setString(0, ip);
    ...
    static bool HandleLookupPlayerAccountCommand(ChatHandler* handler, char const* args)
    {
        ...
        PreparedStatement* stmt = LoginDatabase.GetPreparedStatement(LOGIN_SELECT_ACCOUNT_LIST_BY_NAME);
    ...
    static bool HandleLookupPlayerEmailCommand(ChatHandler* handler, char const* args)
    {
        ...
        PreparedStatement* stmt = LoginDatabase.GetPreparedStatement(LOGIN_SELECT_ACCOUNT_LIST_BY_EMAIL);
    ...
    static bool LookupPlayerSearchCommand(PreparedQueryResult result, int32 limit, ChatHandler* handler)
    {
        if (!result)
        {
            handler->PSendSysMessage(LANGUAGE_NO_PLAYERS_FOUND);
            handler->SetSentErrorMessage(true);
            return false;
        }
        ...
        do
        {
            ...
            PreparedStatement* stmt = CharacterDatabase.GetPreparedStatement(CHARACTER_SELECT_CHARACTER_GUID_NAME_BY_ACC);
            stmt->setUInt32(0, accountId);
            PreparedQueryResult result2 = CharacterDatabase.Query(stmt);

            if (result2)
            {
                handler->PSendSysMessage(LANGUAGE_LOOKUP_PLAYER_ACCOUNT, accountName.c_str(), accountId);
                ...
                do
                {
                    ...
                    handler->PSendSysMessage(LANGUAGE_LOOKUP_PLAYER_CHARACTER, name.c_str(), guid);
                    ++counter;
                }
